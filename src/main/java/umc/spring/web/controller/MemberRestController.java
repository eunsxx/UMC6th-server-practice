...
public class MemberRestController {
    ...
    @PostMapping("/{memberId}/{memberMissionId}/complete")
    @Operation(summary = "미션 상태 업데이트 API", description = "진행 중인 미션을 완료로 변경하는 API입니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200", description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "acess 토큰 만료", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    @Parameters({
            ...
            @Parameter(name = "memberMissionId", description = "회원 미션 ID", required = true),
    })
    public ApiResponse<MissionStatusUpdateResponseDTO.Response> updateMissionStatusToCompleted(@ExistMember @PathVariable Long memberId, @PathVariable Long memberMissionId) {
        MissionStatusUpdateResponseDTO.Response response = memberMissionQueryService.updateMissionStatusToCompleted(memberId, memberMissionId);
